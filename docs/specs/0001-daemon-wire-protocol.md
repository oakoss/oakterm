...
- **Ordering**: within one connection, the daemon answers requests in the order it received them — responses never overtake each other, though serial-0 pushes may interleave between them. Clients may rely on this: a response observed for serial N implies every earlier request on that connection has already been answered. Stateful sequences (`EnterCopyModeAck` gating cache fills, refusals while a `CopyModeInvalidated` push is outstanding) depend on this guarantee; a future daemon that reorders responses per connection is a breaking change.
...
This spec defines protocol version **major 1, minor 5**. Minor bumps are recorded here in implementation order; the advertised `VERSION_MINOR` constant ships with the first implementation of each bump's messages, so binaries may lag the spec by one minor version.
...
| 1.2     | Added `ListTabs` (0xAF) and `TabList` (0xB0) for tab enumeration (tab bar, TREK-107). Additive — unknown to older daemons, which ignore the frame. Also fixed `GetLayoutTree` to resolve `tab_id` literally (TREK-264); pre-1.2 binaries sent a placeholder `tab_id = 0` and relied on the unimplemented resolution serving the active tab — a behavior change, tolerated without a major bump because clients and daemons still ship in lockstep (same exception as 1.5's layout note).                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 1.3     | Added the tab-lifecycle ops `MoveTab` (0xB1), `RenameTab` (0xB2), `RenameWorkspace` (0xB3), and `CloseWorkspace` (0xB4) / `CloseWorkspaceResponse` (0xB5) (tab move/reorder + rename, TREK-209). Additive — unknown to older daemons, which ignore the frame. Ships with `VERSION_MINOR = 3`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 1.4     | `ScrollbackData` (0x74) `start_row` now reports the start the daemon served rather than echoing the request's, so a clamped window's rows key correctly (copy-mode viewport cache, TREK-110). Semantics exception: redefining an existing field is breaking under this spec's own rule, recorded as a minor bump on the same lockstep grounds as 1.2 and 1.5 — no independently versioned client is released, and no shipped client read the field. Nothing on the wire moved: type, width, position, and coordinate space are unchanged, and an unclamped response is byte-identical to the old echo. Ships with `VERSION_MINOR = 4`. Since it is the first advertised minor to postdate the copy-mode messages (0x97-0x9A, cataloged without a version event), a client gates copy mode on minor ≥ 4 — the only version signal that implies both the daemon's pin support and these row-index semantics. |
| 1.5     | The protocol-1.5 batch. (a) Inserted `input_flags` and `kitty_kbd_flags` into the `RenderUpdate` (0x72) payload after `alt_screen` ([Spec-0011](0011-input-encoding.md)) and, with them, `history_len` ([ADR-0025](../adrs/0025-copy-mode-pin-anchor.md)), growing the fixed prefix 25 → 35 bytes. (b) `ScrollbackData` (0x74) gains `base` after `total_rows` (21 → 29 byte prefix): the history length `start_row` was resolved against. (c) Copy-mode entry becomes a serial-correlated request: `EnterCopyMode` (0x97) carries the client's echoed `base`, acked by `EnterCopyModeAck` (0x9B) or refused with an `Error` (`InvalidMessage`) at the serial; `CopyModeInvalidated` (0x9C) is a new push. A client gates copy mode on minor ≥ 5: a 1.4 daemon's decode tolerates trailing bytes, so it would silently ignore the echoed base and pin at its own history end — the race ADR-0025 closes. Layout exception: positional insertions are breaking under this spec's own rule, recorded as a minor bump only because client and daemon still ship in lockstep with no independently versioned client released; once one exists, positional insertions are major bumps. (Renumbered 1.2 → 1.3 → 1.4 → 1.5 as `ListTabs`/`TabList`, the tab-lifecycle ops, and the served-start fix each shipped first — minors are assigned in implementation order.)                                                                                                                                                                                                             |
...
| `0x74`   | ScrollbackData  | D→C       | Response | `pane_id: u32`, `start_row: i64` (the start actually served, not an echo), `has_more: u8`, `total_rows: u32`, `base: u64` (since 1.5, ADR-0025: the history length `start_row` was resolved against — the base a client echoes in `EnterCopyMode` when entering from a fully scrolled page), `rows_len: u32`, `rows: bytes` |
...
which shares the coordinate space of `GetScrollback.start_row`: row 0 is the
first visible row and negative values reference scrollback above it, matching
Spec-0003's selection coordinates and, under a copy-mode pin, the row the
viewport top held at entry. `direction` searches toward older rows
...
**`ScrollbackData.start_row` is what was served.** The daemon clamps a
`GetScrollback` window to the history it holds: a `start_row` reaching past the
oldest retained row is moved forward onto it, and the count is trimmed at the
present. Rows travel the wire positionally — `DirtyRow.row_index` is 0 on every
scrollback row — so `rows[i]` is row `start_row + i` of the response, and the
response's `start_row` is the start the daemon served from, which equals the
request's only when no clamp applied. A client keying a cache off the start it
requested mis-files every row of a clamped window under indices naming other
rows, which is exactly the stable-index guarantee [ADR-0012](../adrs/0012-copy-mode-scrollback-access.md)
exists to provide.
...
alt_screen         u8          1 if the active grid is the alternate screen
                               ...
                               host scrollback.
input_flags        u8          Since 1.5 (Spec-0011). bit0: DECCKM cursor-key
                               mode; bit1: application keypad mode (mode 66);
                               bits2-3: modifyOtherKeys level (0-2);
                               bits4-7: reserved, must be 0.
kitty_kbd_flags    u8          Since 1.5 (Spec-0011). Kitty keyboard
                               progressive-enhancement flags at the top of the
                               active grid's flag stack; 0 = protocol disabled
                               (legacy encoding).
history_len        u64 LE      Since 1.5 (ADR-0025). Rows ever pushed to the
                               pane's scrollback when this update was built —
                               the base a client echoes in EnterCopyMode when
                               entering at offset 0.
...
#### GUI Protocol — Copy Mode (0x97-0x9C)

See Spec-0008 for full copy mode behavior and ADR-0025 for the anchor contract.

| msg_type | Name                | Direction | Serial   | Payload                                                                                                                                                                                                                                                                              |
| -------- | ------------------- | --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `0x97`   | EnterCopyMode       | C→D       | Request  | Since 1.5: `pane_id: u32`, `base: u64`. `base` is the daemon-published history length bound to the content the client painted (`RenderUpdate.history_len` at offset 0, `ScrollbackData.base` when fully scrolled), echoed back as the pin anchor. Validated before any mutation.       |
| `0x9B`   | EnterCopyModeAck    | D→C       | Response | Since 1.5: `pane_id: u32`, `base: u64`. The pin was taken at the echoed base; the client issues no cache fills until this arrives. An out-of-range base is answered with `Error` (`InvalidMessage`) at the serial instead, leaving any existing pin for that client untouched.          |
| `0x98`   | ExitCopyMode        | C→D       | Push (0) | `pane_id: u32`. Unpins the viewport. Scroll position jumps to follow live output.                                                                                                                                                                                                     |
| `0x9C`   | CopyModeInvalidated | D→C       | Push (0) | Since 1.5: `pane_id: u32`. A resize invalidated this client's pin (ADR-0025 clause 5). The client exits copy mode and discards in-flight fills; delivered by the client's own connection task, never consumed by another client's wake.                                                |
...
The daemon tracks which clients have pinned viewports per pane, each mapped to the base its client echoed. Scroll-on-output is suppressed per-client while its ID is in the set. See ADR-0012 and ADR-0025. A pin is released by `ExitCopyMode` or by the client disconnecting; `EnterCopyMode` from an already-pinned client re-pins at the newly echoed base. The daemon accepts `resize_watermark <= base <= history_len()`, where `resize_watermark` is the history length after the last resize that advanced it — a base below it names rows a resize capture invalidated.

**Read refusal while an invalidation is outstanding (since 1.5, ADR-0025 clause 5).** From the moment a resize drops a client's pin until that client's `ExitCopyMode` (or a successful re-pin) lands, the daemon answers that client's `GetScrollback` (0x73), `FindPrompt` (0x75), `SearchScrollback` (0x77), `SearchNext` (0x79), `SearchPrev` (0x7A), and `YankSelection` (0x99) on the pane with `Error` (`InvalidMessage`, "copy-mode pin invalidated by a resize") — never with data resolved against the live fallback. The refusal outlives the `CopyModeInvalidated` push deliberately: a request buffered behind the notification pass must not reach the live coordinate space the client does not know it is in. Clients treat the refusal as unretryable and exit copy mode; the exit clears it.

**Pinned resolution of `GetScrollback` (0x73) and `FindPrompt` (0x75).** Both messages carry `i64` offsets in the coordinate space Spec-0003 defines, counted back from an origin. For a client with no copy-mode pin the origin is the live present, unchanged from Phase 0. For a client holding a pin on that pane the origin is the pinned offset, so its cached row indices stay valid as new output arrives. Both messages must share this rule: they address the same rows, and resolving them against different origins would desync a client's viewport cache from its prompt jumps mid-copy-mode. The payloads are unchanged — resolution depends on daemon-side per-client state, not on any new wire field. A pinned client may pass `start_row >= 0` to reach rows that were on screen when it pinned and have since scrolled into history; unpinned, those offsets still resolve to nothing, since `GetScrollback` never serves live grid rows.

**`ScrollbackData.total_rows` for pinned clients.** `total_rows` is the count of rows that have ever scrolled off the live grid — the exclusive upper bound of the absolute index space, identical for pinned and unpinned clients. It counts rows the daemon can no longer produce: those pruned from the hot buffer while no archive was attached, and those the archive dropped under load (Spec-0004 overload policy). Such rows keep their indices and are served as blank rows, so that later rows do not shift; a client cannot distinguish a genuinely blank row from a lost one, and should treat `total_rows` as addressable range rather than as a guarantee of retained content.

#### GUI Protocol — Split Topology, Tabs & Workspaces (0xA0-0xB5)
...
| `0xB1`   | MoveTab            | C→D       | Push (0) | `tab_id: u32`, `new_index: u32` (0-based target position within the tab's workspace, clamped to the tab count)                                                                                                               |
| `0xB2`   | RenameTab          | C→D       | Push (0) | `tab_id: u32`, `name_len: u16`, `name: UTF-8` (empty clears the pin, reverting to the pane-title fallback)                                                                                                                   |
| `0xB3`   | RenameWorkspace    | C→D       | Push (0) | `workspace_id: u32`, `name_len: u16`, `name: UTF-8`                                                                                                                                                                          |
| `0xB4`   | CloseWorkspace     | C→D       | Request  | `workspace_id: u32`                                                                                                                                                                                                          |
| `0xB5`   | CloseWorkspaceResp | D→C       | Response | Empty. Confirms the workspace and all its panes closed.                                                                                                                                                                      |
...
**`MoveTab` (0xB1) / `RenameTab` (0xB2) / `RenameWorkspace` (0xB3):** Push mutations; the client refreshes with `ListTabs` afterward (same choreography as `SwitchTab`). `MoveTab` reorders within the tab's own workspace only, clamping `new_index` to the tab count; the active tab stays on its own id through the shuffle. `RenameTab` pins an explicit name that overrides the pane-title fallback, and an empty name clears the pin (reverting to the title). Unknown ids fail with `UNKNOWN_TAB` / `UNKNOWN_WORKSPACE`, pushed as `Error` frames with serial 0.

**`CloseWorkspace` (0xB4) / `CloseWorkspaceResponse` (0xB5):** Closes a workspace and every pane in every one of its tabs, shutting down their PTYs, then re-activates the nearest remaining workspace (Spec-0007). The daemon refuses to close the last workspace with `LAYOUT_REJECTED`, keeping the multiplexer non-empty while a client is attached — symmetric with the last-tab (`CloseTab`) and last-pane (`ClosePane`) refusals. An unknown `workspace_id` yields `UNKNOWN_WORKSPACE`.
