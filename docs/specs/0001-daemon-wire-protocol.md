...
| `0x74`   | ScrollbackData  | D→C       | Response | `pane_id: u32`, `start_row: i64` (the start actually served, not an echo), `has_more: u8`, `total_rows: u32`, `rows_len: u32`, `rows: bytes` |
...
The daemon tracks which clients have pinned viewports per pane (a set of client IDs, not a boolean). Scroll-on-output is suppressed per-client while its ID is in the set. See ADR-0012. A pin is released by `ExitCopyMode` or by the client disconnecting; `EnterCopyMode` from an already-pinned client re-pins at the current offset.
