...
This spec defines protocol version **major 1, minor 4**. Minor bumps are recorded here in implementation order; the advertised `VERSION_MINOR` constant ships with the first implementation of each bump's messages, so binaries may lag the spec by one minor version.
