...
use oakterm_terminal::grid::row::{MarkMetadata, Row, SemanticMark};
    ...
    match frame.msg_type {
        ...
        MSG_GET_SCROLLBACK => {
            ...
            if plan.archive_count > 0 {
                ...
                // Errors get an error frame, never blanks: blank rows mean
                // a permanent gap, and a transient failure (wedged writer,
                // EIO) must stay retryable for the client.
                let found = match archive.read_range(plan.archive_start, plan.archive_count) {
                    Ok(found) => found,
                    Err(e) => {
                        warn!(
                            conn_id,
